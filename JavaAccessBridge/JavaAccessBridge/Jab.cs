...
namespace JavaAccessBridge{
    internal class Jab{
        public class AccessibleTreeItem{
            ...
            public AccessibleTreeItem(AccessibleContextInfo accessibleContextInfo) : this(){
        ...
        public static int GetAccessibleSelectionCount(Int32 vmID, IntPtr ac) {
            return JabApi.getAccessibleSelectionCountFromContext(vmID, ac);
        }

        public static bool IsAccessibleChildSelected(Int32 vmID, IntPtr ac,int index) {
            return JabApi.isAccessibleChildSelectedFromContext(vmID, ac,index);
        }
                    ...
                    if (JabApi.getAccessibleContextInfo(vmID, currentPtr, acPtr)){
                        ...
                        if (!ReferenceEquals(currentContext, null)){
                            ...
                            if (!ReferenceEquals(newItem, null)){
                                ...
                                }else if (currentContext.accessibleSelection){
                                    int n = GetAccessibleSelectionCount(vmID, currentPtr);
                                    var sb = new StringBuilder();
                                    for (int i = 0; i < currentContext.childrenCount; i++) {
                                        var b = IsAccessibleChildSelected(vmID, currentPtr,i);
                                        if (b){
                                            sb.Append("1");
                                        } else{
                                            sb.Append("0");
